...
class HomeBoxLayout(BoxLayout):
    ...
    def call_NuevaCalibracion(self):
        nombreCliente = self.ids.NombreCliente.text
        numCertificado = self.ids.NumCertificado.text
        numSolicitud = self.ids.NumSolicitud.text
        numSerie = self.ids.NumSerie.text
        responsable = self.ids.Responsable.text
        revision = self.ids.Revision.text
        patron = self.ids.Patron.text
        material = self.ids.Material.text 
        secuencia = self.ids.Secuencia.text
        numRep  = self.ids.NumRep.text

        moverManual()
        NuevaCalibracion(nombreCliente, numCertificado, numSolicitud, numSerie, responsable, revision, patron, material, secuencia, 0.25, 10, numRep)
